...
class StockDiscussionCrawlerTest {
    ...
    fun testGetDiscussion() {
        ...
        println(discussion)
        assert(discussion.title == "문가가  정말 그랬을까에  자서전 당장 사봐야겠네에  댕큐")
        assert(discussion.date == "2017.06.07T06:33")
        assert(discussion.content == "베트남이 패망한것을 보며 희열을 느꼈다고 자서전에 고백했듯이 625에 김일성이 수백만 명을 죽인 것에 지금도 희열을 느껴 조기를 달지 못하게 했느냐 조기 없는 현충일은 처음본다 625 남침을 대통령이 김정은과 한패가 되어 애국이라고 떠벌이니")
